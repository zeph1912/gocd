...
public class EnvironmentVariableConfig implements Serializable, Validatable, ParamsAttributeAware, PasswordEncrypter, ConfigOriginTraceable {
    ...
    public void validateName(Map<String, EnvironmentVariableConfig> variableConfigMap, ValidationContext validationContext) {
        ...
        if (!currentVariableName.trim().equals(currentVariableName)) {
            configErrors.add(NAME, format("Environment Variable cannot start or end with spaces for %s '%s'.", parentDisplayName, parentName));
            return;
        }
        if (StringUtils.isBlank(currentVariableName)) {
            configErrors.add(NAME, format("Environment Variable cannot have an empty name for %s '%s'.", parentDisplayName, parentName));
            return;
        }
