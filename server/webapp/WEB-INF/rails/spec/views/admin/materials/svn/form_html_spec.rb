...
describe "_form.html.erb" do
  ...
  it "should render all svn material attributes" do
    ...
    expect(response.body).to have_selector(".popup_form input[type='text'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::URL}]'][value='svn://foo']")
    expect(response.body).to have_selector(".popup_form input[type='text'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::USERNAME}]'][value='loser']")
    expect(response.body).to have_selector(".popup_form input[type='password'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::PASSWORD}]'][value='secret']")
    expect(response.body).to have_selector(".popup_form input[type='hidden'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::ENCRYPTED_PASSWORD}]'][value='#{@material_config.getEncryptedPassword()}']", {visible: :hidden})
    expect(response.body).to have_selector(".popup_form input[type='checkbox'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::CHECK_EXTERNALS}]'][checked='checked']")
  ...
  it "should display new svn material view with errors" do
    ...
    error.add(com.thoughtworks.go.config.svn.SvnMaterialConfig::CHECK_EXTERNALS, "Check Externals is wrong")
    error.add(com.thoughtworks.go.config.svn.SvnMaterialConfig::AUTO_UPDATE, "AUTO_UPDATE is wrong")
    ...
    Capybara.string(response.body).find('.popup_form').tap do |popup_form|
      ...
      expect(popup_form).to have_selector("div.field_with_errors input[type='text'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::URL}]'][value='svn://foo']")
      expect(popup_form).to have_selector("div.form_error", :text => "Url is wrong")

      expect(popup_form).to have_selector("div.field_with_errors input[type='text'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::USERNAME}]'][value='loser']")
      expect(popup_form).to have_selector("div.form_error", :text => "Username is wrong")

      expect(popup_form).to have_selector("div.field_with_errors input[type='password'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::PASSWORD}]'][value='secret']")
      expect(popup_form).to have_selector("div.form_error", :text => "Password is wrong")

      expect(popup_form).to have_selector("div.field_with_errors input[type='checkbox'][name='material[#{com.thoughtworks.go.config.svn.SvnMaterialConfig::CHECK_EXTERNALS}]'][checked='checked']")
